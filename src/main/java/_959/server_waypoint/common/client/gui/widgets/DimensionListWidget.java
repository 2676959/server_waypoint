...
public class DimensionListWidget extends ClickableWidget {
    ...
    public DimensionListWidget(int x, int y, int width, int height, TextRenderer textRenderer, Collection<String> dimensionNames, DimensionListCallback callback) {
        super(x, y, width, height, Text.literal("Dimensions list"));
        this.textRenderer = textRenderer;
        scrolledPosition = x;
        this.dimensionNames = new ArrayList<>(dimensionNames);
        this.callback = callback;
    }

    public void updateDimensionNames(Collection<String> dimensionNames) {
        this.dimensionNames.clear();
        this.dimensionNames.addAll(dimensionNames);
        index = 0;
        scrolledPosition = getX();
    }
