...
class RuntimeConfigurationTest {
    ...
    @Test void rejectsUnknownFieldsOversizedFilesAndUnsupportedProtocol() throws Exception {
        ...
        Files.writeString(file, "{\"wrongName\":true,\"otherName\":false}");
        var unknown = assertThrows(IllegalArgumentException.class, () -> RuntimeConfiguration.readBackend(directory));
        assertTrue(unknown.getMessage().contains("otherName, wrongName"));
        assertTrue(unknown.getMessage().contains("remove them or correct their names"));
        Files.writeString(file, "{\"protocolVersion\":999}");
        var protocol = assertThrows(IllegalArgumentException.class, () -> RuntimeConfiguration.readBackend(directory));
        assertTrue(protocol.getMessage().contains("protocolVersion"));
        assertTrue(protocol.getMessage().contains("matching backend and coordinator builds"));
    ...
    @Test void plaintextRequiresLiteralLoopbackAndNoCryptoConfiguration() throws Exception {
        ...
        for (String endpoint : List.of("localhost:25580", "0.0.0.0:25580", "192.0.2.1:25580")) {
            config.addProperty("coordinator", endpoint);
            var failure = assertThrows(IllegalArgumentException.class, () -> RuntimeConfiguration.endpoint(config, "coordinator"));
            assertTrue(failure.getMessage().contains("coordinator"));
            assertTrue(failure.getMessage().contains("literal loopback address"));
        }
