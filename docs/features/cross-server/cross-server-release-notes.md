...
- Vanilla clients can use `/wp remote servers`, `/wp remote list` and `/wp remote tp`. Matching
  protocol-1 modded clients also receive a separate read-only remote manager and confirmation UI.
...
synchronization uses Minecraft custom-payload protocol 1, independently of cross-server protocol v1.
