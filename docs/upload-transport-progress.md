...
| --- | --- | --- |
| Exclusive upload pairing | Resolved | `UploadCoordinator` holds one global 30-second lease. Concurrent uploads return `BUSY`, and an applying upload retains the lease until publication and feedback finish. |
...
| Incomplete upload transport cleanup | Partially resolved | Incomplete transfers expire during ticks, but Paper quit/disable does not yet clear the dedicated upload manager and timeout failure is not request-scoped. |
...
| 2 | Make upload cleanup request-scoped and lifecycle-complete | P1 | Yes | Cancel only an exact player/request pair, clear the dedicated manager on Paper quit/disable, reserve before revision capture, and add a short reacquisition cooldown. |
| 3 | Separate outbound admission from owner-thread delivery | P1/P2 | Yes for user workflows | Admit prepared bodies before scheduling, return an asynchronous final result, cancel failed upload requests exactly, and add an edit-screen response deadline. |
| 4 | Add end-to-end global resource grants | P1 under load | Yes before scale sign-off | Round-robin active peers under one global tick budget and retain inbound accounting through synchronous message application. |
| 5 | Complete automated and live Folia coverage | Release blocker | Yes | Test each boundary above, then run multi-region, disconnect, malformed, saturation, incompatible-client, and low-TPS large-transfer smoke tests. |
...
### 2. Request-scoped upload lifecycle

Every cancellation must compare both player and request ID before releasing the
lease. Paper quit and disable must clear the dedicated upload manager as well as
the general sender manager. Reserve the lease before revision capture, and use a
short cooldown rather than a queued lease if repeated reacquisition needs control.

### 3. Two-stage outbound dispatch
...
### 5. Runtime Folia and stress coverage

Add focused tests for timeout cleanup, request identity, lifecycle shutdown,
asynchronous delivery completion, and global pacing. Then run a Folia smoke test
